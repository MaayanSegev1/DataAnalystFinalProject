...
posFix = ["GK", "RB", "RCB", "RS", "LCB", "LB", "RWB", "RDM", "ST", "RCM", "LWB", "RM", "LDM", "LCM", "LS", "LM", "LAM",
          "RW", "RF", "CAM", "RAM", "CF", "LF", "LW", "CM", "CDM", "CB"]
for pos in posFix:
...
resuToSave.loc[resuToSave["Value"].str.contains("K"), "Value"] = resuToSave["Value"].str.split("K").str[0].astype(
    float) / 1000
resuToSave["Value"] = round(resuToSave["Value"].astype(float), 2)
resuToSave = resuToSave.loc[resuToSave["Value"] > 0]
resuToSave.loc[resuToSave["Wage"].str.contains("K"), "Wage"] = resuToSave["Wage"].str.split("K").str[0].astype(
    float) * 1000
resuToSave["Wage"] = round(resuToSave["Wage"].astype(float) / 1000000, 5)
resuToSave.loc[resuToSave["ReleaseClause"].str.contains("K"), "ReleaseClause"] = \
resuToSave["ReleaseClause"].str.split("K").str[0].astype(float) / 1000
...
FixDataPlayer = FixDataPlayer[['Value', 'Overall', 'Age', 'Name', 'Cross', 'Jumping', 'BestOverall',
                               ...
                               'Reaction', 'Balance', 'Power', 'PowerShoT', 'Team', 'Dribbling', 'Strength',
                               'LongShots',
                               'StandingTackle', 'Positioning', 'Defending', 'Marking', 'Mentali', 'SlidingTackle',
                               'Goalkeeping',
...
ColPlayerListAttritube = FixDataPlayer[
    ['ValueLog', 'Overall', 'TotalStats', 'ReleaseClause', 'BestOverall', 'IntReputation']]
...
ColPlayerListAttritube = FixDataPlayer[
    ['ValueLog', 'Overall', 'TotalStats', 'ReleaseClause', 'BestOverall', 'Age', 'IntReputation']]
...
print(y_pred2)
df = pd.DataFrame({'Name': data.Name, 'Actual Value': ynew, 'Predicted Value': y_pred2, 'Age': data.Age,
                   'Positioning': data.Positioning}).head(40)
print(df)
...
df = pd.DataFrame({'Name': data.Name, 'Actual Overall': ynew, 'Predicted Overall': y_pred2, 'Age': data.Age,
                   'Positioning': data.Positioning}).head(40)
print(df)
